...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h4 class="page-title mb-0"><i class="bi bi-person-badge me-2"></i>Antrenörler</h4>
</div>

<div class="card mb-3"><div class="card-body">
<form id="filter-form" class="row g-2 align-items-end">
  <div class="col-md-5"><label class="form-label form-label-sm">Ara</label><input id="search" class="form-control form-control-sm" placeholder="Ad, soyad, email"></div>
...
<thead><tr><th>#</th><th>Ad Soyad</th><th>E-posta</th><th>Telefon</th><th>Uzmanlık</th><th>Şube</th><th>Durum</th></tr></thead>
<tbody id="trainers-body"><tr><td colspan="7" class="text-center py-4 text-muted">Yükleniyor...</td></tr></tbody>
</table>
</div></div></div>
...
async function loadTrainers() {
  ...
  try {
    const payload = await apiGet('/gym/backend/api/trainers.php', { search, status, limit: 100 });
    ...
    tbody.innerHTML = rows.map(row => `
      <tr>
        ...
        <td><span class="badge ${row.availability_status === 'active' ? 'badge-aktif' : 'badge-pasif'}">${row.availability_status === 'active' ? 'Aktif' : 'Pasif'}</span></td>
      </tr>
    `).join('');
  } catch (error) {
